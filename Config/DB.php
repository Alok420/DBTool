    ...
    while ($row = $list->fetch_assoc()) {
      ...
      <div class="profile-container" style=" margin: auto;
        ...
        <h1 style="text-align: center;">
          ...
          <img style="margin: auto;" src="../img/<?php echo $table; ?>/<?php echo $row["image"] == "avatar-male.png" ?: $row["image"]; ?>" class="img-circle img-responsive" height="100" width="100">
          <?php
...
//--------------------end of displaying name instead of showing id------------------------------------------------
        ...
        if ($externallinks == "Update ad") {
          $link = "<td><div><a class='btn btn-success' href='user_order_detail.php?id=" . $row['id'] . "'>$externallinks </a></div></td>";
          echo $link;
        }
        if ($externallinks == "Invoice") {
          $link = "<td><div><a href='invoice.php?id=" . $row['id'] . "'>$externallinks </a></div></td>";
          echo $link;
        }
    ...
    public function getAllTableNameFromDB($dbname = "") {
      ...
      if ($dbname == "") {
        ...
        while ($table = $tables->fetch_assoc()) {
          array_push($tablelist, $table["Tables_in_eng"]);
        }
      } else {
        $tables = $this->conn->query("show tables from $dbname");
        while ($table = $tables->fetch_assoc()) {
          array_push($tablelist, $table[0]);
        }
      }
      return $tablelist;
    }
